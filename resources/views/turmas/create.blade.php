...
@yield('page_title', 'Cadastrar Turma')
...
<div class="mb-6 flex items-center gap-3">
    ...
    <div>
        <h2 class="text-xl font-bold text-slate-800">Nova Turma Escolar</h2>
        <p class="text-xs text-slate-400 mt-1">Registre uma nova turma configurando o nome, turno e ano.</p>
    </div>
</div>

<div class="max-w-2xl bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
    <form action="{{ route('turmas.store') }}" method="POST" class="p-8 space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="sm:col-span-2">
                ...
                <input type="text" name="nome_turma" id="nome_turma" value="{{ old('nome_turma') }}" required
            ...
            <div>
                ...
                <select name="periodo" id="periodo" required
                    ...
                    <option value="Matutino" {{ old('periodo') === 'Matutino' ? 'selected' : '' }}>Matutino (Manhã)</option>
                    <option value="Vespertino" {{ old('periodo') === 'Vespertino' ? 'selected' : '' }}>Vespertino (Tarde)</option>
                </select>
                @error('periodo')
                <p class="text-xs text-rose-500 mt-1.5 flex items-center gap-1"><i class="bx bxs-error-circle"></i> {{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="ano" id="ano" value="{{ old('ano', date('Y')) }}" min="2000" max="2100" required
