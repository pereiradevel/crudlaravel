...
<div class="mb-6 flex items-center gap-3">
    ...
    <div>
        <h2 class="text-xl font-bold text-slate-800">Editar Usuário: {{ $user->name }}</h2>
        <p class="text-xs text-slate-400 mt-1">Atualize as credenciais ou o nível de acesso do usuário.</p>
    </div>
</div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="sm:col-span-2">
                ...
                <select name="role" id="role" required
                    ...
                    <option value="owner" {{ old('role', $user->role) === 'owner' ? 'selected' : '' }}>Gestor Dono (Acesso total)</option>
                    <option value="admin" {{ old('role', $user->role) === 'admin' ? 'selected' : '' }}>Administrador (Gerencia Cadastros)</option>
                    <option value="user" {{ old('role', $user->role) === 'user' ? 'selected' : '' }}>Comum (Apenas leitura)</option>
                </select>
                @error('role')
                <p class="text-xs text-rose-500 mt-1.5 flex items-center gap-1"><i class="bx bxs-error-circle"></i> {{ $message }}</p>
                @enderror
            ...
            <div class="sm:col-span-2 p-4 rounded-xl bg-slate-50 border border-slate-100 text-xs text-slate-500">
                <i class="bx bx-info-circle text-indigo-500 align-middle text-sm mr-1"></i>
                Deixe os campos de senha em branco se não quiser alterar a senha atual do usuário.
            </div>

            <div>
                <label for="password" class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Nova Senha (Opcional)</label>
            ...
            <div>
                <label for="password_confirmation" class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Confirmar Nova Senha</label>
